...
from TelethonCat.DB.husb_sql import (add_hus_grp, get_all_hus_grp, is_husb,
                                     rm_hus_grp)
from TelethonCat.DB.waifu_sql import add_grp, get_all_grp, is_harem, rm_grp
...
@dcat_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@dcat_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, hell_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autowaifu works in groups only !!")
