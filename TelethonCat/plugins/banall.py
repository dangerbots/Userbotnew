...
@dcat_cmd(pattern="kickall$", allow_sudo=False)
async def _(event):
...
@dcat_cmd(pattern="banall$", allow_sudo=False)
async def _(event):
    ForGo10God, _, _ = await client_id(event)
    result = await event.client(
        functions.channels.GetParticipantRequest(event.chat_id, ForGo10God)
...
@dcat_cmd(pattern="blockall(?:\s|$)([\s\S]*)")
...
@dcat_cmd(pattern="blocknc(?:\s|$)([\s\S]*)")
...
@dcat_cmd(pattern ="unblockall(?:\s|$)([\s\S]*)")
